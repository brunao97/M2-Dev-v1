...
class InventoryWindow(ui.ScriptWindow):
	...
	def RefreshBagSlotWindow(self):
		...
		for i in xrange(player.INVENTORY_PAGE_SIZE):
			...
			# Check for other active items (pets, mounts, etc.)
			elif self.__IsActiveItem(itemVnum, slotNumber):
				if slotNumber >= player.INVENTORY_PAGE_SIZE * self.inventoryPageIndex:
					slotNumber -= player.INVENTORY_PAGE_SIZE * self.inventoryPageIndex
				self.wndItem.ActivateSlot(slotNumber)
			else:
				# Deactivate slot for non-active items
				if slotNumber >= player.INVENTORY_PAGE_SIZE * self.inventoryPageIndex:
					slotNumber -= player.INVENTORY_PAGE_SIZE * self.inventoryPageIndex
				self.wndItem.DeactivateSlot(slotNumber)
	...
	def __IsActiveItem(self, itemVnum, slotNumber):
		# Check if item is currently active (being used)
		# This includes mounts and other special active items

		# Check if it's a mount/ride item currently being used
		# if player.IsMountingHorse():
		# 	# Get the mount item vnum being used
		# 	mountVnum = player.GetItemIndex(player.EQUIPMENT_SLOT_HORSE)
		# 	if mountVnum == itemVnum:
		# 		return True

		# NOTE: Pet activation requires server-side support to know which pet is active
		# For now, pets won't show activation border until proper server communication is implemented

		# Add more active item checks here as needed
		# Examples: special costumes, active skills, etc.

		return False

	def __IsPetItem(self, itemVnum):
		# Check if item is a pet item
		# This is a basic check - you may need to adjust based on your item database
		pet_item_ranges = [
			(53001, 54000),  # Example pet item range
			(55001, 56000),  # Another example range
			# Add more ranges as needed for your specific pet items
		]

		for start, end in pet_item_ranges:
			if start <= itemVnum <= end:
				return True
...
#			print "Belt Global Pos : ", self.wndBelt.GetGlobalPosition()
	...
	if app.BL_ENABLE_PICKUP_ITEM_EFFECT:
		def ActivateSlot(self, slotindex, type):
			...
			self.wndItem.ActivateSlot(slotindex)
