...
# ListBoxExNew for ENABLE_SEND_TARGET_INFO
if app.ENABLE_SEND_TARGET_INFO:
	class ListBoxExNew(Window):
		...
		def __init__(self, stepSize, viewSteps):
			...
			self.itemList=[]
			self.scrollBar=None
			self.SetWindowName("NONAME_ListBoxExNew")
		...
		def SetBasePos(self, basePos, refresh=True):
			if basePos<0:
				basePos=0

			scrollLen=self.maxSteps-self.viewSteps
			if basePos>scrollLen:
				basePos=scrollLen

			self.basePos=basePos
			self.baseIndex=basePos
			
			self.__RefreshItems()

			if refresh and self.scrollBar:
				scrollLen=self.__GetScrollLen()
				if scrollLen>0:
					self.scrollBar.SetPos(float(basePos) / float(scrollLen))

		def __RefreshItems(self):
			basePos=self.basePos
			stepsToGo=self.viewSteps
			curY=0

			for item in self.itemList:
				item.SetPosition(0, curY)
				item.ResetCurrentHeight()
				item.Show()

				if basePos>0:
					removeTop = min(basePos, item.GetHeight() / self.stepSize)
					if removeTop == item.GetHeight() / self.stepSize:
						item.Hide()
					
					item.SetRemoveTop(removeTop * self.stepSize)
					basePos -= removeTop

				if stepsToGo <= 0:
					item.Hide()
				else:
					stepsLeft=item.GetCurrentHeight() / self.stepSize
					if stepsLeft > stepsToGo:
						item.SetRemoveBottom((stepsLeft - stepsToGo)*self.stepSize)
						stepsToGo = 0
					else:
						stepsToGo -= stepsLeft

					curY+=item.GetCurrentHeight()
